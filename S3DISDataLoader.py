...
##现在输入是xyzrgb和语义标签
##我要的输入是xyzrgb和点击点和前景标签
class S3DISDataset(Dataset):
    def __init__(self, split='train', data_root='trainval_fullarea', num_point=4096, test_area=5, block_size=1.0, sample_rate=1.0, transform=None,pre_prompt=[],pre_seg=[]):
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        instance_num = np.max(labels)
        N_points = points.shape[0]
        
        while (True):
            choose_instance = np.random.randint(1,instance_num+1)
